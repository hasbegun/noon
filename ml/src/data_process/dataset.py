...
class FoodDataset(Dataset):
    ...
    def __init__(
        ...
        # Filter valid samples - both file exists and can be loaded
        logger.info(f"Filtering valid images from {len(self.df)} samples...")
        valid_mask = self.df["image_path"].apply(self._is_valid_image)
        self.df = self.df[valid_mask].reset_index(drop=True)
        logger.info(f"Kept {len(self.df)} valid samples")
    ...
    def _is_valid_image(self, image_path_str: str) -> bool:
        ...
        try:
            ...
            # Loading 70K+ images during init takes 3+ minutes!
            return image_path.exists() and image_path.stat().st_size > 0
        except Exception:
            return False

    def __getitem__(self, idx: int) -> Dict[str, Any]:
        """Get a sample from the dataset - NEVER returns None"""
        try:
            ...
            # If image failed to load, return dummy sample immediately
            if image is None:
            ...
            if self.transform:
                try:
                    ...
                    logger.error(f"Transform failed for {image_path}: {e}, returning dummy sample")
                    return self._create_dummy_sample()
            ...
            logger.error(f"Error loading sample {idx}: {e}, returning dummy sample")
            return self._create_dummy_sample()

    def _create_dummy_sample(self) -> Dict[str, Any]:
        """Create a dummy sample when all loading attempts fail"""
        from config import config

        # Create a black image
        dummy_image = np.zeros((config.image_size, config.image_size, 3), dtype=np.uint8)
        dummy_mask = np.zeros((config.image_size, config.image_size), dtype=np.uint8)

        # Apply transforms to get proper tensor format
        if self.transform:
            transformed = self.transform(image=dummy_image, mask=dummy_mask)
            dummy_image = transformed["image"]
            dummy_mask = transformed["mask"]
        else:
            # Manually create tensors if no transform
            dummy_image = torch.from_numpy(dummy_image).permute(2, 0, 1).float()
            dummy_mask = torch.from_numpy(dummy_mask).long()
